#!/bin/bash -l
#SBATCH --job-name=sceneweaver_idlayout
#SBATCH --output=slurm_logs/sceneweaver_idlayout_%j.out
#SBATCH --error=slurm_logs/sceneweaver_idlayout_%j.err
#SBATCH --time=6:50:00
...
MODEL_STORAGE_ROOT="${MODEL_STORAGE_ROOT:-${SCENEWEAVER_MODEL_STORAGE_ROOT}}"
...
AUTO_FIX_TRANSFORMERS="${AUTO_FIX_TRANSFORMERS:-1}"
MIN_TRANSFORMERS_VERSION="${MIN_TRANSFORMERS_VERSION:-4.41.0}"
TRANSFORMERS_PIP_PACKAGES="${TRANSFORMERS_PIP_PACKAGES:-transformers==4.41.2 tokenizers==0.19.1 sentencepiece tiktoken}"
...
IDENTITY_DRIFT_THRESHOLD="${IDENTITY_DRIFT_THRESHOLD:-0.70}"
IDENTITY_PENALTY_SCALE="${IDENTITY_PENALTY_SCALE:-0.25}"
LAYOUT_DRIFT_THRESHOLD="${LAYOUT_DRIFT_THRESHOLD:-0.65}"
LAYOUT_PENALTY_SCALE="${LAYOUT_PENALTY_SCALE:-0.20}"
...
OUTPUT_DIR="${OUTPUT_DIR:-outputs/story_run_identity_layout_${RUN_STAMP}}"
...
if [ "${AUTO_FIX_TRANSFORMERS}" = "1" ]; then
...
def _parse(v):
    ...
    while len(out) < 3:
        out.append(0)
    return tuple(out[:3])
ok = _parse(transformers.__version__) >= _parse("${MIN_TRANSFORMERS_VERSION}")
sys.exit(0 if ok else 1)
PY
  then
    echo "Upgrading transformers stack for Qwen2.5 support (need >= ${MIN_TRANSFORMERS_VERSION})."
    # shellcheck disable=SC2206
    TF_PACKAGES_ARR=(${TRANSFORMERS_PIP_PACKAGES})
    "${PYTHON_BIN}" -m pip install --upgrade "${TF_PACKAGES_ARR[@]}"
  fi
fi
...
PIPELINE_SCRIPT="${PIPELINE_SCRIPT:-scripts/run_story_pipeline_identity_layout.py}"
...
if supports_flag "--identity_drift_threshold"; then
  CMD+=(--identity_drift_threshold "${IDENTITY_DRIFT_THRESHOLD}")
fi
if supports_flag "--identity_penalty_scale"; then
  CMD+=(--identity_penalty_scale "${IDENTITY_PENALTY_SCALE}")
fi
if supports_flag "--layout_drift_threshold"; then
  CMD+=(--layout_drift_threshold "${LAYOUT_DRIFT_THRESHOLD}")
fi
if supports_flag "--layout_penalty_scale"; then
  CMD+=(--layout_penalty_scale "${LAYOUT_PENALTY_SCALE}")
fi
...
echo "IDENTITY_DRIFT_THRESHOLD=${IDENTITY_DRIFT_THRESHOLD}"
echo "IDENTITY_PENALTY_SCALE=${IDENTITY_PENALTY_SCALE}"
echo "LAYOUT_DRIFT_THRESHOLD=${LAYOUT_DRIFT_THRESHOLD}"
echo "LAYOUT_PENALTY_SCALE=${LAYOUT_PENALTY_SCALE}"
...
echo "AUTO_FIX_TRANSFORMERS=${AUTO_FIX_TRANSFORMERS}"
echo "MIN_TRANSFORMERS_VERSION=${MIN_TRANSFORMERS_VERSION}"
echo "TRANSFORMERS_PIP_PACKAGES=${TRANSFORMERS_PIP_PACKAGES}"
