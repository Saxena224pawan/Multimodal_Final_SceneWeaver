...
def _apply_drift_penalties(
    *,
    ...
    identity_penalty_scale: float,
    layout_penalty_scale: float,
) -> Dict[str, Any]:
    identity_drift = None
    identity_penalty = 0.0
    if transition_similarity is not None:
        identity_drift = max(0.0, float(identity_threshold) - float(transition_similarity))
        identity_penalty = identity_drift * max(0.0, float(identity_penalty_scale))

    layout_drift = None
    layout_penalty = 0.0
    if environment_similarity is not None:
        layout_drift = max(0.0, float(layout_threshold) - float(environment_similarity))
        layout_penalty = layout_drift * max(0.0, float(layout_penalty_scale))

    total_penalty = identity_penalty + layout_penalty
    adjusted_score = max(0.0, float(base_score) - total_penalty)

    notes: List[str] = []
    if identity_drift is not None and identity_drift > 0.0:
        notes.append("Increase character identity consistency with previous window.")
    if layout_drift is not None and layout_drift > 0.0:
        notes.append("Improve adjacent-window layout consistency (background/object placement/camera framing).")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.70,
        help="Minimum preferred first-frame identity similarity to previous last-frame embedding.",
    )
    parser.add_argument(
        "--identity_penalty_scale",
        type=float,
        default=0.25,
        help="Penalty weight applied when identity similarity falls below identity_drift_threshold.",
    )
    parser.add_argument(
        "--layout_drift_threshold",
        type=float,
        default=0.65,
        help="Minimum preferred adjacent-window layout/environment similarity.",
    )
    parser.add_argument(
        "--layout_penalty_scale",
        type=float,
        default=0.20,
        help="Penalty weight applied when layout similarity falls below layout_drift_threshold.",
    )
    parser.add_argument(
        "--critic_story_weight",
        type=float,
    ...
    for window_pos, window in selected_windows:
        ...
        if not args.dry_run:
            ...
            identity_threshold = max(-1.0, min(1.0, float(args.identity_drift_threshold)))
            identity_penalty_scale = max(0.0, float(args.identity_penalty_scale))
            layout_threshold = max(-1.0, min(1.0, float(args.layout_drift_threshold)))
            layout_penalty_scale = max(0.0, float(args.layout_penalty_scale))
            if scene_change_requested:
                scene_change_decay = max(0.0, float(args.scene_change_env_decay))
                environment_weight *= scene_change_decay
                layout_threshold *= scene_change_decay
            if args.parallel_window_mode:
                previous_beat = ""
            else:
            ...
            for attempt_idx in range(max_attempts):
                ...
                for candidate_idx in range(num_candidates):
                    ...
                    drift = _apply_drift_penalties(
                        base_score=critic.final_score,
                        transition_similarity=transition_similarity,
                        environment_similarity=environment_similarity,
                        identity_threshold=max(identity_threshold, transition_floor),
                        layout_threshold=layout_threshold,
                        identity_penalty_scale=identity_penalty_scale,
                        layout_penalty_scale=layout_penalty_scale,
                    )
                    adjusted_critic_score = drift["adjusted_score"]
                    combined_feedback = critic.feedback
                    if drift["notes"]:
                        combined_feedback = f"{combined_feedback} {drift['notes']}".strip()
                    ...
                    candidate_entry = {
                        ...
                        "environment_similarity": environment_similarity,
                        "continuity_score": continuity_score,
                        "critic_score_raw": critic.final_score,
                        "critic_score_adjusted": adjusted_critic_score,
                        "critic_story_progress_score": critic.story_progress_score,
                        "identity_drift": drift["identity_drift"],
                        "identity_penalty": drift["identity_penalty"],
                        "layout_drift": drift["layout_drift"],
                        "layout_penalty": drift["layout_penalty"],
                        "drift_penalty_total": drift["total_penalty"],
                    ...
                    candidate_state = {
                        ...
                        "critic_score_raw": critic.final_score,
                        "critic_score": adjusted_critic_score,
                        "critic_feedback": combined_feedback,
                        "identity_drift": drift["identity_drift"],
                        "identity_penalty": drift["identity_penalty"],
                        "layout_drift": drift["layout_drift"],
            ...
            row["selected_critic_score_raw"] = selected["critic_score_raw"]
            row["selected_critic_feedback"] = selected["critic_feedback"]
            row["selected_identity_drift"] = selected["identity_drift"]
            row["selected_identity_penalty"] = selected["identity_penalty"]
            row["selected_layout_drift"] = selected["layout_drift"]
            row["selected_layout_penalty"] = selected["layout_penalty"]
            row["selected_drift_penalty_total"] = selected["drift_penalty_total"]
            ...
            row["identity_drift_threshold"] = max(identity_threshold, transition_floor)
            row["layout_drift_threshold"] = layout_threshold
            row["identity_penalty_scale"] = identity_penalty_scale
            row["layout_penalty_scale"] = layout_penalty_scale
    ...
    summary = {
        ...
        "identity_drift_threshold": args.identity_drift_threshold,
        "identity_penalty_scale": args.identity_penalty_scale,
        "layout_drift_threshold": args.layout_drift_threshold,
        "layout_penalty_scale": args.layout_penalty_scale,
