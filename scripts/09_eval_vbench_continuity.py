...
def find_latest_story_run(outputs_root: Path) -> Path:
    ...
    for candidate in reversed(candidates):
        ...
        if clips_dir.is_dir():
            has_video = any(clips_dir.glob("window_*.mp4")) or any(clips_dir.glob("window_*.mov")) or any(clips_dir.glob("window_*.mkv")) or any(clips_dir.glob("window_*.avi"))
            if has_video:
                return clips_dir
        has_video = any(candidate.rglob("*.mp4")) or any(candidate.rglob("*.mov")) or any(candidate.rglob("*.mkv")) or any(candidate.rglob("*.avi"))
        if has_video:
            return candidate
    raise RuntimeError(f"No non-empty story runs found under {outputs_root.as_posix()}")


def ensure_cuda_visible(ngpus: int) -> None:
    if ngpus <= 0:
        return
    import socket
    import torch

    if torch.cuda.is_available() and torch.cuda.device_count() >= ngpus:
        return

    host = socket.gethostname()
    visible = torch.cuda.device_count()
    raise RuntimeError(
        f"VBench continuity requires {ngpus} visible GPU(s), but torch sees {visible} on host {host}. "
        "Run this under sbatch/salloc on a GPU node, or set --ngpus 0 only if you intentionally patched VBench for CPU execution."
    )
...
def main() -> None:
    ...
    if not args.dry_run:
        ensure_cuda_visible(args.ngpus)
