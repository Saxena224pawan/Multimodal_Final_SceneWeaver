...
def find_latest_story_run(outputs_root: Path) -> Path:
    ...
    for candidate in reversed(candidates):
        ...
        if clips_dir.is_dir():
            return candidate
    raise RuntimeError(f"No story run with a clips/ directory found under {outputs_root.as_posix()}")
    ...
    *,
    ...
    if not run_log_path.is_file():
        raise FileNotFoundError(
            f"run_log.jsonl not found at {run_log_path.as_posix()}; cannot recover per-window prompts."
        )
    ...
    if missing_log_windows:
        raise RuntimeError(
            "Missing run_log.jsonl rows for window indices: "
            f"{missing_log_windows}. Expected one row per window clip."
        )
    ...
    meta = {
        "run_log_path": run_log_path.as_posix(),
...
def generate_interpretation_report(run_dir: Path, summary: dict, manifest: dict) -> Tuple[Path, Path]:
    ...
    for row in summary.get("results", []):
        lines.append(
            ...
            f"mode=`{row.get('mode', '')}`, elapsed=`{fmt_seconds(float(row.get('elapsed_seconds', 0.0)))}`"
        )
